...
const TOKEN_EXPIRATION = 5 * 60 * 1000; // 5 mins
  ...
  const handleLogin = async () => {
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        const correctNetwork = isDev ? BSC_TESTNET.chainId : BSC_MAINNET.chainId;
        ...
        const expectedChainId = BigInt(isDev ? BSC_TESTNET.chainId : BSC_MAINNET.chainId, 16); // Convert from hex string to decimal
        ...
        if (currentChainId !== expectedChainId) {
          ...
          const resp = await axios.get(`http://localhost:3333/api/auth/pubKey/${pubKey}/rid/1`);
